...
Class Master extends DBConnection {
	...
	function save_client(){
		...
		extract($_POST);
		$data = "";
        $username = $this->conn->real_escape_string($_POST['user_name']); // Extract and sanitize the username
		foreach($_POST as $k =>$v){
			if(!in_array($k,array('id'))){
				if(!is_numeric($v))
					$v = $this->conn->real_escape_string($v);
	...
	function get_expiration(){
		extract($_POST);
        $new_date = date_create($registration_date);
        date_add($new_date, date_interval_create_from_date_string("{$duration} months"));

        $day = date_format($new_date, "d");

        if ($day > 15) {
            $last_day_of_month = date_format($new_date, "Y-m-t");
            $res = $last_day_of_month;
        } else {
            // If the day is 15 or less, pick the last day of the previous month
            date_sub($new_date, date_interval_create_from_date_string("$day days"));
            $res = date_format($new_date, "Y-m-t");
        }
...
//        $_POST['user_name'] = $userName;
		...
		if(empty($id)){
//			$sql = "INSERT INTO `insurance_list` set {$data}, `user_name` = '{$username}' ";
            $sql = "INSERT INTO `insurance_list` set {$data} ";
		}else{
//			$sql = "UPDATE `insurance_list` set {$data}, `username` = '{$username}' where id = '{$id}' ";
		...
		if($save){
            ...
            $resp['err'] = $this->conn->error . "[" . $sql . "]";
            // Log the error
            error_log("Database Error: " . $resp['err']);
		}
		if($resp['status'] =='success')
			$this->settings->set_flashdata('success',$resp['msg']);
		return json_encode($resp);
