        ...
        <div class="container-fluid">
			...
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
					...
					$qry = $conn->query("SELECT i.*, CONCAT(p.code, ' - ', p.name) as `policy`, p.duration, p.doc_path, c.name as category, CONCAT(cc.code, ' - ', cc.lastname, ', ', cc.firstname, ' ', cc.middlename) as client
                   FROM `insurance_list` AS i
                   INNER JOIN `policy_list` AS p ON i.policy_id = p.id
                   INNER JOIN `category_list` AS c ON p.category_id = c.id
                    INNER JOIN `client_list` AS cc ON i.client_id = cc.id
                    WHERE  DATE(i.registration_date) BETWEEN '{$from}' AND '{$to}'
                    ORDER BY DATE(i.registration_date) ASC");
                    ...
                    INNER JOIN `category_list` AS c ON p.category_id = c.id
                    INNER JOIN `client_list` AS cc ON i.client_id = cc.id
                    WHERE i.user_name = '$user'
                    AND DATE(i.registration_date) BETWEEN '{$from}' AND '{$to}'
                   ORDER BY DATE(i.registration_date) ASC");
