...
<div class="card card-outline card-primary rounded-0 shadow">
    <div class="card-header">
        <h3 class="card-title">Expires Soon</h3>
        ...
        <div class="container-fluid">
            <div class="container-fluid">
                <table class="table table-bordered table-hover table-striped">
                    ...
                    echo $userName;
                    $userName1 = "Administrator Admin";
                    $i = 1;
                    $clients = $conn->query("SELECT *,CONCAT(lastname,', ', firstname,' ', middlename) as fullname FROM client_list where id in (SELECT client_id from `insurance_list`) ");
...
ORDER BY DATE(`registration_date`) ASC;");
                    ...
                    while($row = $qry->fetch_assoc()):
                        ...
                        <tr>
                            ...
                            <td class=""><p class="m-0 truncate-1"><?php echo $row['expiration_date'] ?></p></td>
                            ...
                            if ($_settings->userdata('type') == 1):
                                ?>
                                <td class=""><p class="m-0 truncate-1"><?php echo $row['user_name']; ?></p></td>
                            <?php
                            endif;
                            ?>
                            <td class="text-center">
                                ...
                                if (isset($row['expiration_date'])) {
                                    $expirationDate = date('Y-m-d', strtotime($row['expiration_date']));
                                    $oneWeekBefore = date('Y-m-d', strtotime('-7 days', strtotime($expirationDate)));
//echo $oneWeekBefore;
//echo "---";
//echo date('yY-m-d');
//echo "---";
//echo $expirationDate;
                                    if (date('Y-m-d') == $oneWeekBefore) {
                                        echo '<span class="rounded-pill badge badge-warning bg-gradient-warning px-3">One Week Before</span>';
                                    } elseif ($expirationDate < date('Y-m-d')) {
                                        echo '<span class="rounded-pill badge badge-danger bg-gradient-danger px-3">Expired</span>';
                                    } else {
                                        switch ($row['status']) {
                                            case 1:
                                                echo '<span class="rounded-pill badge badge-success bg-gradient-warning px-3">Expires Soon</span>';
                                                break;
                                            case 0:
                                    ...
                                    // Handle the case where expiration_date is not set
                                    echo '<span class="rounded-pill badge badge-light bg-gradient-light border px-3">N/A</span>';
                                }
                                ?>
                            </td>
