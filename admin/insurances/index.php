			...
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
					...
					$qry = $conn->query("SELECT * from `insurance_list` order by date(registration_date) asc  ");
						...
						$qry = $conn->query("SELECT * from `insurance_list` WHERE `user_name` = '" . $conn->real_escape_string($userName) . "'order by date(registration_date) asc  ");
								...
								<td class="text-center">
									...
									if (isset($row['expiration_date'])) {
										...
										if ($currentDate < $expirationDate) {
											$dateDifference = strtotime($expirationDate) - strtotime($currentDate);
											if ($dateDifference < 8 * 24 * 3600) { // 8 days in seconds
												echo '<span class="rounded-pill badge badge-warning bg-gradient-warning px-3">Expires Soon</span>';
											} else {
