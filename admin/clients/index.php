			...
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
					...
					$qry = $conn->query("SELECT *,CONCAT(lastname, ', ', firstname, ' ', middlename) as fullname from `client_list` where delete_flag = 0 order by CONCAT(lastname, ', ', firstname, ' ', middlename) asc ");
						...
						$qry = $conn->query("SELECT *,CONCAT(lastname, ', ', firstname, ' ', middlename) as fullname from `client_list` where delete_flag = 0 AND `user_name` = '" . $conn->real_escape_string($userName) . "'order by CONCAT(lastname, ', ', firstname, ' ', middlename) asc ");
